...
fn install_event_api(ctx: &mut Context) {
    ...
    // ── KeyboardEvent (caniuse: 99%+) ──
    let kb_ctor = NativeFunction::from_copy_closure(|_this, args, ctx| {
        let type_ = arg_string(args, 0);
        let obj = boa_engine::object::JsObject::with_object_proto(ctx.intrinsics());
        ...
        let _ = obj.insert_property(boa_engine::js_string!("type"), pd(JsValue::from(boa_engine::js_string!(type_))));
        let _ = obj.insert_property(boa_engine::js_string!("key"), pd(JsValue::from(boa_engine::js_string!(""))));
        let _ = obj.insert_property(boa_engine::js_string!("code"), pd(JsValue::from(boa_engine::js_string!(""))));
        let _ = obj.insert_property(boa_engine::js_string!("location"), pd(JsValue::from(0u32)));
        let _ = obj.insert_property(boa_engine::js_string!("ctrlKey"), pd(JsValue::from(false)));
        let _ = obj.insert_property(boa_engine::js_string!("shiftKey"), pd(JsValue::from(false)));
        let _ = obj.insert_property(boa_engine::js_string!("altKey"), pd(JsValue::from(false)));
        let _ = obj.insert_property(boa_engine::js_string!("metaKey"), pd(JsValue::from(false)));
        let _ = obj.insert_property(boa_engine::js_string!("repeat"), pd(JsValue::from(false)));
        let _ = obj.insert_property(boa_engine::js_string!("isComposing"), pd(JsValue::from(false)));
        let _ = obj.insert_property(boa_engine::js_string!("charCode"), pd(JsValue::from(0u32)));
        let _ = obj.insert_property(boa_engine::js_string!("keyCode"), pd(JsValue::from(0u32)));
        let _ = obj.insert_property(boa_engine::js_string!("which"), pd(JsValue::from(0u32)));
        let _ = obj.insert_property(boa_engine::js_string!("bubbles"), pd(JsValue::from(false)));
        ...
        // initKeyboardEvent (chromium non-standard).
        let init_kb = NativeFunction::from_copy_closure(|_t, _a, _ctx| Ok(JsValue::undefined()));
        let _ = obj.insert_property(boa_engine::js_string!("initKeyboardEvent"), pd(JsValue::from(boa_engine::object::FunctionObjectBuilder::new(ctx.realm(), init_kb).build())));
        let _ = obj.insert_property(boa_engine::js_string!("initKeyEvent"), pd(JsValue::from(boa_engine::object::FunctionObjectBuilder::new(ctx.realm(), NativeFunction::from_copy_closure(|_t, _a, _ctx| Ok(JsValue::undefined()))).build())));
        Ok(obj.into())
    });
    let _ = ctx.register_global_callable(boa_engine::js_string!("KeyboardEvent"), 1, kb_ctor);

    // ── MouseEvent (caniuse: 99%+) ──
    let ms_ctor = NativeFunction::from_copy_closure(|_this, args, ctx| {
        let type_ = arg_string(args, 0);
        let obj = boa_engine::object::JsObject::with_object_proto(ctx.intrinsics());
        let pd = |v: JsValue| { boa_engine::property::PropertyDescriptor::builder().value(v).writable(true).enumerable(true).configurable(true).build() };
        let _ = obj.insert_property(boa_engine::js_string!("type"), pd(JsValue::from(boa_engine::js_string!(type_))));
        let _ = obj.insert_property(boa_engine::js_string!("screenX"), pd(JsValue::from(0u32)));
        let _ = obj.insert_property(boa_engine::js_string!("screenY"), pd(JsValue::from(0u32)));
        let _ = obj.insert_property(boa_engine::js_string!("clientX"), pd(JsValue::from(0u32)));
        let _ = obj.insert_property(boa_engine::js_string!("clientY"), pd(JsValue::from(0u32)));
        let _ = obj.insert_property(boa_engine::js_string!("ctrlKey"), pd(JsValue::from(false)));
        let _ = obj.insert_property(boa_engine::js_string!("shiftKey"), pd(JsValue::from(false)));
        let _ = obj.insert_property(boa_engine::js_string!("altKey"), pd(JsValue::from(false)));
        let _ = obj.insert_property(boa_engine::js_string!("metaKey"), pd(JsValue::from(false)));
        let _ = obj.insert_property(boa_engine::js_string!("button"), pd(JsValue::from(0u32)));
        let _ = obj.insert_property(boa_engine::js_string!("buttons"), pd(JsValue::from(0u32)));
        let _ = obj.insert_property(boa_engine::js_string!("relatedTarget"), pd(JsValue::null()));
        let _ = obj.insert_property(boa_engine::js_string!("movementX"), pd(JsValue::from(0u32)));
        let _ = obj.insert_property(boa_engine::js_string!("movementY"), pd(JsValue::from(0u32)));
        let _ = obj.insert_property(boa_engine::js_string!("detail"), pd(JsValue::from(0u32)));
        let _ = obj.insert_property(boa_engine::js_string!("bubbles"), pd(JsValue::from(false)));
        let _ = obj.insert_property(boa_engine::js_string!("cancelable"), pd(JsValue::from(false)));
        let _ = obj.insert_property(boa_engine::js_string!("target"), pd(JsValue::null()));
        let _ = obj.insert_property(boa_engine::js_string!("defaultPrevented"), pd(JsValue::from(false)));
        let init_ms = NativeFunction::from_copy_closure(|_t, _a, _ctx| Ok(JsValue::undefined()));
        let _ = obj.insert_property(boa_engine::js_string!("initMouseEvent"), pd(JsValue::from(boa_engine::object::FunctionObjectBuilder::new(ctx.realm(), init_ms).build())));
        Ok(obj.into())
    });
    let _ = ctx.register_global_callable(boa_engine::js_string!("MouseEvent"), 1, ms_ctor);

    // ── FocusEvent (caniuse: 99%+) ──
    let fo_ctor = NativeFunction::from_copy_closure(|_this, args, ctx| {
        let type_ = arg_string(args, 0);
        let obj = boa_engine::object::JsObject::with_object_proto(ctx.intrinsics());
        let pd = |v: JsValue| { boa_engine::property::PropertyDescriptor::builder().value(v).writable(true).enumerable(true).configurable(true).build() };
        let _ = obj.insert_property(boa_engine::js_string!("type"), pd(JsValue::from(boa_engine::js_string!(type_))));
    ...
    let _ = ctx.register_global_callable(boa_engine::js_string!("FocusEvent"), 1, fo_ctor);
